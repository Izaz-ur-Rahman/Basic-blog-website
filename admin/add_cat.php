...
if (isset($_POST['add_cat'])) {
    ...
    if($row)
    {
        $msg = ['Category name already exist','alert-danger']; // just simple array mai nai used ki.
        $_SESSION['msg'] = $msg;
        header("location: add_cat.php");
    }
    else
    {
        $sql2 = "INSERT INTO categories (cat_name) value ('$cat_name')";
        $query2 = mysqli_query($config,$sql2);
        if ($query2)
        {
            $msg = ['Category Successfully inserted💪💪💪','alert-success'];  // just simple array mai nai used ki.
            $_SESSION['msg'] = $msg;
            header("location: add_cat.php");
